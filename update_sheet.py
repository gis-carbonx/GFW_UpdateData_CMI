...
def fuse_sources(df):
    ...
    gdf = gpd.GeoDataFrame(df,
        ...
        crs="EPSG:4326").to_crs(epsg=32749)
...
def cluster_points_by_owner(gdf):
    ...
    for (owner, tanggal), group in gdf.groupby(["Owner","Integrated_Date"]):
        ...
        group = group.copy()
        group["buffer"] = group.geometry.buffer(30)
    ...
    if not results:
        print("Cluster gagal → fallback aktif")
        gdf = gdf.to_crs(4326)
        gdf["Cluster_ID"] = "NO_CLUSTER"
        gdf["Cluster_Y"] = gdf.geometry.y
        gdf["Cluster_X"] = gdf.geometry.x
        gdf["Jumlah_Titik"] = 1
        return gdf
...
def overwrite_google_sheet(df):
    ...
    df = df[existing_cols].fillna("").astype(str)

    print("FINAL ROW:", len(df))

    data = [existing_cols] + df.values.tolist()
    sheet.append_rows(data, value_input_option="USER_ENTERED")

    print("Berhasil tulis ke Google Sheet")
    ...
    if not df_raw.empty:
        ...
        if not df.empty:
            ...
            if not gdf.empty:
                ...
                print("Gagal: gdf kosong")
        else:
            print("Gagal: AOI kosong")
    else:
